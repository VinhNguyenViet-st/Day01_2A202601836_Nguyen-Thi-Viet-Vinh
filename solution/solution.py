...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def shorten(value: str, max_length: int = 40) -> str:
    ...
    for result in results:
        rows.append([
            ...
            f'{result["gpt4o_latency"]:.4f}s',
            f'{result["mini_latency"]:.4f}s',
        ])

    # Tìm độ rộng phù hợp cho từng cột.
    column_widths = []
    ...
    for column_index, header in enumerate(headers):
        ...
        column_widths.append(max(len(value) for value in values))
